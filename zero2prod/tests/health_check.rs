...
use uuid::Uuid;
use zero2prod::configuration::{get_configuration, DatabaseSettings, Setting};
...
pub async fn configure_database(config: &DatabaseSettings) -> PgPool {
    let mut connections = PgConnection::connect(&config.connection_string_without_db())
        .await
...
async fn subscribe_returns_a_200_for_vaild_form_data() {
    ...
    let saved = sqlx::query!("SELECT email, name FROM subscriptions")
        ...
        .await
        .expect("Faild to fetch saved subscriptions");
